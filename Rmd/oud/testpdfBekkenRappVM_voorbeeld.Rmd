...
mystyle <- list(theme_bw(), 
                theme(plot.title = element_text(size = rel(0.8), color = "black")))
```

#Inleiding

Het Veerse Meer is in 2003 van een brakwaterbekken veranderd in een zoutwaterbekken. Dit heeft effect gehad op zoutgehalte, maar ook op nutrienten en zuurstofconcentraties. 
...
Het Veerse Meer is een belangrijk Natura2000 gebied (@Waterbeheer2010, p.35 )
...
De gemiddelde saliniteit over de hele periode was `r round(mean(subdf$waarde, na.rm = T),2)` PSU. Voor de opening van de Veersrme Gatdam was dit gemiddeld `r round(mean(subdfvoor$waarde, na.rm = T),2)` PSU, erna `r round(mean(subdfna$waarde, na.rm = T),2)` PSU
