...
Tabel 5.1. Overzicht van data voor saliniteit in het Veerse Meer. bron: MWTL via Servicedesk data en meetpalen via [link](http://waterberichtgeving.rws.nl/nl/water-en-weer_dataleveringen_ophalen-opgetreden-data.htm)
...
```{r, echo=FALSE, warning = F, fig.height = 4, fig.width = 8, fig.cap="figuur 5.x"}
...
In deze grafieken worden de wintermetingen weergeven door de horizontale blauwe lijn, de rode horizontale lijnen geven de zomermeting weer.
...
In de periode `r beginjaar` - `r eindjaar` nam de fosfaat concentratie nam af met ongeveer 0.25 mg P/jaar (zie figuur).
...
In de periode `r beginjaar` - `r eindjaar` nam de nitraat concentratie af met ongeveer 0.8 mg N/jaar (zie figuur). 
...
In de periode `r beginjaar` - `r eindjaar` nam de silicaat concentratie af met 2.5 mg Si/jaar (zie figuur). 

```{r, echo = F, warning = F, fig.height = 2, fig.width = 8, eval = F}
...
In de periode `r beginjaar` - `r eindjaar` nam de ammonium concentratie af met 0.2 mg NH3/jaar (zie figuur). 
