...
# [1] "extinctie coefficient"                        "silicaat"                                    
# [3] "saliniteit"                                   ""                                            
# [5] "nitraat"                                      "nitriet"                                     
# [7] "ammonium"                                     "totaal particulair stikstof"                 
# [9] "totaal particulair fosfaat"                   "temperatuur"                                 
#[11] "Chloride in mg/l in oppervlaktewater"         "gesuspendeerd materiaal"                     
#[13] "totaal organisch koolstof"                    "opgelost fosfaat"                            
#[15] "zuurgraad"                                    "totaal opgelost fosfaat"                     
#[17] "totaal fosfaat"                               "zuurstof"                                    
#[19] "totaal opgelost stikstof"                     "totaal stikstof"                             
#[21] "kjelldahl stikstof"                           "chlorofyl-a"                                 
#[23] "Percentage zuurstof in % in oppervlaktewater" "opgelost organisch koolstof"                 
#[25] "particulair organisch koolstof"     

trend.plot <- function (subdf, variable, xx, yy, unit = "eenheid", trendline = F, loessline = F, lspan = 0.3, perc90line = F, pointline = F) {
...
subdf = subset(rws_dat, rws_dat$variable == "gesuspendeerd materiaal" &
                 ...
                 rws_dat$year >= from & rws_dat$year <= to)
trend.plot(subdf, "variable", xx = "datetime", yy = "waarde", unit = "eenheid")

```

Zwevende stof in het Veerse Meer varieerde in de periode `r from` tot `r to` tussen `r round(min(subdf$waarde, na.rm = T), 1)` en `r round(max(subdf$waarde, na.rm = T), 1)` mg/l (Figuur 5.1).
...
```{r, echo = F, fig.height = 2, fig.width = 8}
subdf = subset(rws_dat, rws_dat$variable == "nitraat" &
                 rws_dat$locatie == "Soelekerkepolder oost" &
                 rws_dat$year >= from &
                 rws_dat$year <= to)
trend.plot(subdf, "variable", xx = "datetime", yy = "waarde", unit = "eenheid", trendline = T)

```

In de periode `r from` - `r to` nam de nitraat concentratie af/toe met xxx mg N/jaar (zie figuur). 
...
subdf = subset(rws_dat, rws_dat$variable == "opgelost fosfaat" &
                 ...
                 rws_dat$year >= 2005 &
                 rws_dat$year <= 2015)
trend.plot(subdf, "variable", xx = "datetime", yy = "waarde", unit = "eenheid", loessline = T, lspan = 0.3)
...
subdf = subset(rws_dat, rws_dat$variable == "silicaat" &
                 ...
                 rws_dat$year >= 2010 &
                 rws_dat$year <= 2015)
trend.plot(subdf, "variable", xx = "datetime", yy = "waarde", unit = "eenheid", perc90line = T)
...
subdf = subset(rws_dat, rws_dat$variable == "ammonium" &
                 ...
                 rws_dat$year >= 2000 &
                 rws_dat$year <= 2015)
trend.plot(subdf, "variable", xx = "datetime", yy = "waarde", unit = "eenheid", pointline = T)
